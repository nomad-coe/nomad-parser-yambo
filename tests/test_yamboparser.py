...
def test_0(parser):
    ...
    parser.parse('tests/data/hBN/r_setup', archive, None)
...
def test_1(parser):
    archive = EntryArchive()
    parser.parse('tests/data/hBN/r-10b_1Ry_HF_and_locXC_gw0_em1d_ppa', archive, None)

    run = archive.run[-1]
    method = run.method
    assert len(method) == 5
    assert method[0].x_yambo_transferred_momenta[0].x_yambo_input[0].x_yambo_parameters['Sigma scattering'] == 'yes'
    assert method[1].x_yambo_dipoles[0].x_yambo_input[0].x_yambo_file == './SAVE//ns.kb_pp_pwscf'
    assert method[1].x_yambo_dipoles[0].x_yambo_input[0].x_yambo_sn == '009327'
    assert method[1].x_yambo_dipoles[0].x_yambo_output[0].x_yambo_parameters['Fragmentation'] == 'yes'
    assert method[2].x_yambo_dynamic_dielectric_matrix[0].x_yambo_output[0].x_yambo_parameters['BZ Q point size factor'] == approx(1.0)
    assert method[3].x_yambo_local_xc_nonlocal_fock[0].x_yambo_output[0].x_yambo_parameters['RL vectors'] == 1491
    assert method[4].x_yambo_dyson[0].x_yambo_input[0].x_yambo_parameters['X poles'] == approx(100.)
    assert method[4].x_yambo_dyson[1].x_yambo_output[0].x_yambo_parameters['GW solver'] == 'Newton'
    ...
    assert calc[0].x_yambo_bosonic_temperature == approx(0.)
...
def test_2(parser):
    archive = EntryArchive()
    parser.parse('tests/data/Aluminum/r-01_Lifetimes_em1d_life', archive, None)
...
def test_3(parser):
    archive = EntryArchive()
    parser.parse('tests/data/GaSb/r-02_GW_em1d_ppa_HF_and_locXC_gw0', archive, None)

    method = archive.run[-1].method
    assert len(method) == 4
    assert method[0].x_yambo_transferred_momenta[0].x_yambo_input[0].x_yambo_parameters['QP states'] == '1  29'
    assert method[2].x_yambo_bare_xc[0].x_yambo_output[0].x_yambo_parameters['Bosonic    Temperature'] == approx(0.)
    ...
    assert calc[1].x_yambo_bare_xc_bandenergies[0].x_yambo_dft[0][0][4].magnitude == approx(-1.57994179e-18)
...
def test_4(parser):
    archive = EntryArchive()
    parser.parse('tests/data/LiF/r-02_QP_PPA_em1d_ppa_HF_and_locXC_gw0', archive, None)
